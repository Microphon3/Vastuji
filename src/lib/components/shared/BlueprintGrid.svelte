...
<div class="blueprint-grid grid grid-cols-4 grid-rows-4 gap-0 w-full aspect-square border-2 border-[var(--grid-lines)]">
  {#each displayZones as zone (zone.id)}
    <button
      data-zone={zone.id}
      class="zone-cell relative border border-[var(--grid-lines)] transition-all duration-200 {getStatusColor(zone.status)} {interactive ? 'cursor-pointer' : 'cursor-default'}"
      class:interactive
      onclick={() => handleZoneClick(zone.id)}
      disabled={!interactive}
    >
      <!-- Zone number -->
      <span class="absolute top-1 left-1 text-xs font-mono text-gray-400">
        {zone.id}
      </span>

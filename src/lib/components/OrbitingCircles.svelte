...
>
	{#each children as iconName, i}
		<div
			class="bg-opacity-20 border-opacity-30 absolute flex items-center justify-center rounded-full border border-orange-500 bg-black backdrop-blur-sm"
			style="
				width: {iconSize}px;
				height: {iconSize}px;
				top: 50%;
				left: 50%;
				transform: translate(-50%, -50%) rotate({(360 / itemCount) *
				i}deg) translateY(-{radius}px) rotate(-{(360 / itemCount) * i}deg);
			"
		>
